...
`, base64.StdEncoding.EncodeToString(raw))), 0666)
	...
	rawSmiles, err := ioutil.ReadFile("fig/old_man_smiles_at.png")
	require.NoError(t, err)
	ioutil.WriteFile("packed_smiles.go", []byte(fmt.Sprintf(
		`// Copyright 2021 oncilla
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
...
var rawOldManSmiles = []byte(%q)
`, base64.StdEncoding.EncodeToString(rawSmiles))), 0666)
}
