...
func main() {
	...
	cmd := &cobra.Command{
		...
		Short: executable + " makes Abe smile at stuff!",
		Long: `Enjoy Abe smiling at stuff!

Provide a target image and Abe Simpson will smile at it.

By default, the resulting image is created in the current working directory
as 'old-man-smiles-at-<target-basename>.png'. If Abe should redirect his smiling,
you have the following options:

  - <filename>.png: Create image at the specified filename.
  - png: Create image at 'old-man-smiles-at-<target-basename>.png'.
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return smile(args[0], flags.out, flags.size)
		},
	}
	cmd.AddCommand(
		newCompletion(cmd),
...
func smile(filename, output string, size uint) error {
	input, err := loadImage(filename)
	if err != nil {
		return err
	}
	m := yeller.SmileAt(input)
	...
	case output == "png":
		...
		name = "old-man-smiles-at-" + name + ".png"
